...
class PrayerTimesRepository {
     ...
     * is passed in. The Hijri date comes from the same online response — shifted by
     * [hijriCorrectionDays] via the API's own `adjustment` parameter, which shifts only the
     * Hijri label and leaves the prayer times themselves untouched. If the network is
     * unreachable or the request fails for any reason, silently falls back to
    ...
    private fun fetchOnline(lat: Double, lon: Double, method: PrayerCalculationMethod, hijriCorrectionDays: Int): PrayerTimesState {
        ...
        val url = "https://api.aladhan.com/v1/timings/$dateStr?latitude=$lat&longitude=$lon" +
            "&method=${method.aladhanMethodId}&midnightMode=1&adjustment=$hijriCorrectionDays"
        ...
        client.newCall(request).execute().use { response ->
            ...
            val hijri = hijriJson?.let {
                HijriDate(
                    ...
                    monthNumber = it.optJSONObject("month")?.optInt("number", 0) ?: 0
                )
            }
